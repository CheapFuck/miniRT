...
static void	parse_sphere_atributes(t_sphere *sphe, char **tokens, char **colors)
{
	sphe->material.color.r = ft_atoi(colors[0]);
	sphe->material.color.g = ft_atoi(colors[1]);
	sphe->material.color.b = ft_atoi(colors[2]);
	sphe->material.checker = ft_atoi(tokens[4]);
	sphe->material.reflectivity = ft_atof(tokens[5]);
	sphe->material.transparency = ft_atof(tokens[6]);
	sphe->material.refractive_index = ft_atof(tokens[7]);
}
		...
		t_scene *scene)
{
	...
	parse_sphere_atributes(&sphere, tokens, colors);
